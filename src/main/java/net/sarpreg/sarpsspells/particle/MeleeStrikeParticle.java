...
public class MeleeStrikeParticle extends TextureSheetParticle {
    private final SpriteSet sprites;
    private final Vec3 forward;
    private final boolean mirror, vertical;
    private final Vector3f[] localVertices;

    MeleeStrikeParticle(ClientLevel pLevel, double pX, double pY, double pZ, SpriteSet spriteSet, double xd, double yd, double zd, FlameStrikeParticleOptions options) {
    ...
    private void createEmberTrail() {
        ...
        for (int i = 1; i < particleCount - 1; i++) {
            ...
            level.addParticle(ParticleTypes.POOF, x + localPos.x, y + localPos.y, z + localPos.z, 0, 0, 0);
        }
    }
    ...
    public static class Provider implements ParticleProvider<FlameStrikeParticleOptions> {
        ...
        public Particle createParticle(@NotNull FlameStrikeParticleOptions options, @NotNull ClientLevel pLevel, double pX, double pY, double pZ, double pXSpeed, double pYSpeed, double pZSpeed) {
            MeleeStrikeParticle shriekparticle = new MeleeStrikeParticle(pLevel, pX, pY, pZ, sprite, pXSpeed, pYSpeed, pZSpeed, options);
