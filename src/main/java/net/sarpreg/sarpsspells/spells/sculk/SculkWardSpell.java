...
public class SculkWardSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(SarpsSpellsMod.MODID, "sculk_ward");
        ...
        for (int r = 0; r < rings; r++) {
            ...
            for (int i = 0; i < fangs; i++) {
                ...
                if (!world.getBlockState(BlockPos.containing(spawn).below()).isAir()) {
                    SculkTendril tendril = new SculkTendril(world, entity, getDamage(spellLevel, entity));
                    tendril.moveTo(spawn.x, spawn.y, spawn.z, get2DAngle(center, spawn), 0);
                    world.addFreshEntity(tendril);
                }
            }
        }
