...
public class EmergingTentaclesSpell extends AbstractSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(SarpsSpellsMod.MODID, "emerging_tentacles");

    @Override
    public List<MutableComponent> getUniqueInfo(int spellLevel, LivingEntity caster) {
        return List.of(
                Component.translatable("ui.irons_spellbooks.damage", Utils.stringTruncation(getDamage(spellLevel, caster), 2)),
                Component.translatable("ui.sarps_spells.tentaclecount", getCount(spellLevel, caster))
        );
    }
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (level.getBlockState(BlockPos.containing(spawn).below()).isFaceSturdy(level, BlockPos.containing(spawn).below(), Direction.UP)) {
                VoidTentacle spike = new VoidTentacle(level, entity, getDamage(spellLevel, entity));
                if (i % 2 == count % 2) {
